...
router.delete('/:codReceita', (req,res) => {
    ...
    const query = 'DELETE FROM tbreceitas WHERE codREceitas = ?';
...
router.put('/:codReceita', (req,res) => {
    const {codReceita} = req.params;
    const {rendePorcoes,nomeReceita} = req.body;
    const query = 'UPDATE tbreceitas SET rendePorcoes = ?, nomeReceita = ? WHERE codReceita = ?';

    dbConnection.query(query, [rendePorcoes,nomeReceita,codReceita], (err,result) => {
       if(err) throw err;
       res.status(201).json({
